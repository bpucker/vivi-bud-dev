...
__usage__ = """ 	python per_gene_inspection.py
...
def analyze_correlation( exp_data, genes, results_per_gene_file ):
	...
	genes_to_plot = []
	with open( results_per_gene_file, "w" ) as out:
		out.write( "GeneID\tSpearmanCorrelationCoefficient\tp-value\tAverageExpressionCPM\n" )
		for gene in genes:
			...
			for idx, date in enumerate( dates_order[1:] ):
				for i in range( 3 ):
					try:
						y_values.append( exp[ date ][ i ] )
						x_values.append( temp_data[ ticks[ idx ] ] )
					except IndexError:
						pass
			
			r, p = stats.spearmanr( x_values, y_values )
			out.write( "\t".join( map( str, [ gene + "\t" + str( r ) + "\t" + str( p ) + "\t" + str( np.median( y_values ) ) ] ) ) + "\n" )
			if r > 0.3 and p < 0.05:
				genes_to_plot.append( gene )
	return genes_to_plot
	

def plot_expression( exp_data, genes, directory  ):
	...
	for gene in genes:
		figure_file = directory + gene + ".png"
		...
		for idx, date in enumerate( dates_order ):
			y_values.append( np.mean( exp_data[ gene ][ date ] ) )
			x_values.append( dates_order.index( date ) )
			ax.plot( [ ticks[ idx ], ticks[ idx ] ], [ min( exp_data[ gene ][ date ] ), max( exp_data[ gene ][ date ] ) ], color="grey" )
...
def main(arguments):
	...
	results_per_gene_file = directory + "results_per_gene.txt"
	...
	genes_to_plot = analyze_correlation( exp_data, genes, results_per_gene_file )
	plot_expression( exp_data, genes_to_plot, directory  )
