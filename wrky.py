...
def construct_heatmap( datamatrix, genes, tissues, heatmap_file ):
	...
	sns.heatmap( df, vmin=0, vmax= my_vmax, ax=ax, linewidths=0.3, annot=True, annot_kws={'fontsize':4}, cbar=False, fmt="g", cmap='YlGnBu' )	#binary	#cmap='YlGnBu'  = 1
	
	for idx, gene in enumerate( genes ):
		ax.text( -4.5, idx+0.75, gene, fontsize=5 )
	
	for idx, tissue in enumerate( tissues ):
		ax.text( idx+0.4, len( genes )+0.5, "20"+tissue[-2:] + "-" + tissue[2:4] + "-" + tissue[:2], rotation=90, fontsize=5 )
	...
	plt.subplots_adjust( left=0.2, right=0.99, top=0.99, bottom=0.1, wspace=0.2 )
...
if __name__ == '__main__':
	...
	heatmap_file = prefix + "AdditionalFileF.pdf"
